...
* Instead of the concatenated string in C#, use 
```C#
...
""";
    ...
    {
        command.Paramters.AddWithValue("@searchText", searchString);
        try
        {
```


* Never use a $"" concatenate string with user input in a select (or any) sql statemenet. See Demo in code example. BELOW IS WHAT  MAKES INJECTION POSSIBLE, ABOVE IS OK
